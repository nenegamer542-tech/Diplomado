...
Se implementó trazabilidad por lote y serie dentro de inventario, propagada a los flujos existentes de compra, venta y producción. El conteo físico persiste snapshots de stock y detalle, concilia cantidades/identificadores/vencimientos, detecta cambios concurrentes y publica movimientos idempotentes por línea con compensación ante error. QA global: 29 suites y 461 pruebas aprobadas; Fase 3 aprobada en código y QA.
...
| Inventario | Stock, umbrales/alertas, trazabilidad lote/serie, compras/ventas/producción y conteo físico agregado y trazable | Kardex y procesos completos | Revisión visual de flujo frontend de conteo físico | La operación multi-documento usa compensaciones; una caída abrupta en medio de la publicación requiere reintento y revisión del conteo parcial | Alta |
