...
const CREATE_FIELDS = ['code', 'name', 'taxId', 'email', 'phone', 'notes', 'status'];
...
const createSchema = z
  .object({
    code: z
      .string()
      .trim()
      .min(1, 'El código del cliente es obligatorio.')
      .max(20)
      .regex(/^[A-Za-z0-9._-]{1,20}$/, 'El código admite letras, números, puntos, guiones y guion bajo.'),
    name: z.string().trim().min(2, 'El nombre del cliente debe tener al menos 2 caracteres.').max(120),
