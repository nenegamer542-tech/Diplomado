'use strict';

const mongoose = require('mongoose');

/**
 * SUPPLIER — Proveedores de una empresa.
 * Colección: suppliers (siempre companyId-scoped)
 *
 * `code` es único POR empresa (mayúsculas). El borrado está bloqueado si el
 * proveedor tiene órdenes de compra (se desactiva en su lugar).
 */
const supplierSchema = new mongoose.Schema(
  {
    companyId: { type: mongoose.Schema.Types.ObjectId, ref: 'Company', required: true, index: true },
    code: {
      type: String,
      required: 'El código del proveedor es obligatorio.',
      trim: true,
      uppercase: true,
      maxlength: 20,
    },
    name: { type: String, required: 'El nombre del proveedor es obligatorio.', trim: true, maxlength: 120 },
    contactName: { type: String, trim: true, maxlength: 120, default: null },
    email: { type: String, trim: true, lowercase: true, maxlength: 120, default: null },
    phone: { type: String, trim: true, maxlength: 30, default: null },
    address: { type: String, trim: true, maxlength: 200, default: null },
    notes: { type: String, trim: true, maxlength: 500, default: null },
    status: { type: String, enum: ['active', 'inactive'], default: 'active' },
  },
  { timestamps: true, strict: true }
);

supplierSchema.index({ companyId: 1, code: 1 }, { unique: true });
supplierSchema.index({ companyId: 1, status: 1 });

module.exports = mongoose.model('Supplier', supplierSchema);
