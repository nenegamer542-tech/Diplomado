...
const list = asyncHandler(async (req, res) => {
  ...
  Object.assign(extra, searchFilterMulti(['code', 'name', 'email'], req.query.search));

  const filter = tenantFilter(req, extra);
  const { items, total } = await supplierService.list(filter, { sort, skip, limit });
...
const create = asyncHandler(async (req, res) => {
  const supplier = await supplierService.create(req.body, req.user.companyId);
  return created(res, supplier);
});

const update = asyncHandler(async (req, res) => {
  const before = await supplierService.getById(req.params.id, req.user.companyId);
  if (!before) throw ApiError.notFound('Recurso no encontrado.');
  req.auditBefore = before;

  const after = await supplierService.update(req.params.id, req.body, req.user.companyId);
...
const remove = asyncHandler(async (req, res) => {
  ...
  const after = await supplierService.remove(req.params.id, req.user.companyId);
